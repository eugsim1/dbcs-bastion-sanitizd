...
## create priviledges for DataSafe on Autonomous
...
##
cat << ADWPRIV > run_privilegesADWDATASAFE.sh
source env-tns
$ADWSQL << EOF 
@adw_datasafe_priv.sql
EOF
ADWPRIV

### we dont change the DATA_SAFE ACCOUNT ANY MORE LEGACY STUFF
### dont uncomment
###
cat << ALLINPUT
source env-tns
...
$ADWSQL << EOF 
@dscs_privileges.sql  DS\$ADMIN  GRANT ALL -VERBOSE
EOF
END_COMMENT
...
select count(*)  as nub_entries_countries   from COUNTRIES ;
select count(*)  as nub_entries_DEPARTMENTS from DEPARTMENTS ;
select count(*)  as nub_entries_EMPLOYEE    from EMPLOYEES S;
select count(*)  as nub_entries_JOBS        from JOBS ;
select count(*)  as nub_entries_JOB_HISTORY from JOB_HISTORY ;
select count(*)  as nub_entries_LOCATIONS   from LOCATIONS ;
select count(*)  as nub_entries_REGIONS     from REGIONS ;
EOF
